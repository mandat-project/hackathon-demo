...
async function createResourceInAnyRegistrationOfShape(webId, shapeTreeUri, resourceBody, session) {
    if (session === undefined)
        session = new mandat_shared_solid_oidc_1.Session();
    const offerContainerUris = (await getDataRegistrationContainers(webId, shapeTreeUri, session))[0];
    return await (0, mandat_shared_solid_requests_1.createResource)(offerContainerUris, resourceBody, session);
}
...
function getRegistrySet(webId, session) {
    ...
    return getResourceAsStore(webId, session).then((store) => store
        .getObjects(null, (0, mandat_shared_solid_requests_1.INTEROP)("hasRegistrySet"), null)
        .map((term) => term.value));
}
function getDataRegistry(registrySetUri, session) {
    ...
    return getResourceAsStore(registrySetUri, session).then((store) => store
        .getObjects(null, (0, mandat_shared_solid_requests_1.INTEROP)("hasDataRegistry"), null)
        .map((term) => term.value));
}
async function getDataRegistrations(dataRegistryUri, session) {
    ...
    return getResourceAsStore(dataRegistryUri, session).then((store) => store
        .getObjects(null, (0, mandat_shared_solid_requests_1.INTEROP)("hasDataRegistration"), null)
        .map((term) => term.value));
}
function getRegisteredShapeTree(dataRegistrationUri, session) {
    if (session === undefined)
        session = new mandat_shared_solid_oidc_1.Session();
    return getResourceAsStore(dataRegistrationUri, session).then((store) => store.getObjects(null, (0, mandat_shared_solid_requests_1.INTEROP)("registeredShapeTree"), null)[0].value);
}
