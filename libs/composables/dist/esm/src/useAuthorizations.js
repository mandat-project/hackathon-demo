...
const accessRequestInformationResources = ref([]);
// keep track of access receipts
const accessReceiptInformationResources = ref([]);
const handledAccessRequests = ref([]);
const accessRequests = computed(() => accessRequestInformationResources.value.filter(r => !handledAccessRequests.value.map(h => h.split('#')[0]).includes(r)));
// keep track of which children access authorizations are already revoked
const emptyAuthorizations = ref([]);
const shapeTreesOfMissingDataRegs = ref([]);
const replacedAccessAuthorizations = ref([]);
/**
...
const _revokeAccessAuthorizationEvents = ref([]);
...
const createdAccessReceipts = reactive(new Map());
// Access Groups Maps
const createdAccessAuthorization = reactive(new Map());
// Data Authorization Maps
const createdDataAuthorization = reactive(new Map());
const dataAuthzContainer = ref(''); // computed(() => storage.value + dataAuthzContainerName + "/");
const accessAuthzContainer = ref(''); // computed(() => storage.value + accessAuthzContainerName + "/");
const accessAuthzArchiveContainer = ref(''); // computed(() => storage.value + accessAuthzArchiveContainerName + "/");
const accessReceiptContainer = ref(''); // computed(() => storage.value + accessReceiptContainerName + "/");
 ...
 * @param redirect
 */
export async function useAccessRequest(uri, redirect) {
 ...
 * @param redirect
 */
export async function useAccessReceipt(uri, redirect) {
 ...
 */
export async function useAccessNeedGroup(uri, forSocialAgents) {
 ...
 * @param uri
 */
export async function useAccessAuthorization(uri, redirect) {
    ...
    const revokedDataAuthorizations = ref([]);
    /**
    ...
    async function _removeDataAuthorizationsAndCreateNewAccessAuthorization(dataAuthorizations) {
        ...
        resourceStore.addQuad(new NamedNode(newLocation + "#" + accessAuthzLocale), new NamedNode(INTEROP("replaces")), new NamedNode(archivedLocation + "#" + accessAuthzLocale));
        ...
        resourceStore.addQuad(new NamedNode(newLocation + "#" + accessAuthzLocale), new NamedNode(INTEROP("grantedAt")), dateLiteral);
        // in new resource, remove link to data authorization
        for (const dataAuthorization of dataAuthorizations) {
            resourceStore.removeQuads(resourceStore.getQuads(new NamedNode(newLocation + "#" + accessAuthzLocale), new NamedNode(INTEROP("hasDataAuthorization")), dataAuthorization, null));
 ...
 * @param forSocialAgents
 */
export async function useAccessNeed(uri, forSocialAgents) {
    const { memberOf } = useSolidProfile();
    const store = await _fetchStoreOf(uri);
    ...
    async function _checkIfMatchingDataRegistrationExists() {
        const dataRegistrations = await getDataRegistrationContainers(`${memberOf.value}`, registeredShapeTrees[0], session.value).catch((err) => {
            console.error({
                severity: "error",
    ...
    async function grantDataAuthorization() {
        ...
        for (const shapeTree of registeredShapeTrees) {
            const dataRegistrations = await getDataRegistrationContainers(`${memberOf.value}`, shapeTree, session.value).catch((err) => {
                console.error({
                    severity: "error",
                    summary: "Error on getDataRegistrationContainers!",
                    detail: err,
...
_:rename a solid:InsertDeletePatch;
    solid:inserts {
        ...
        await patchResource(aclURI, patchBody, session.value).catch((err) => {
            console.error({
                severity: "error",
 ...
 * @param forSocialAgents
 */
export async function useDataAuthorization(uri) {
    const { memberOf } = useSolidProfile();
    const resourceStore = await _fetchStoreOf(uri);
