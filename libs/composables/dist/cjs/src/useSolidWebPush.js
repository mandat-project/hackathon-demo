...
const _getSolidWebPushDetails = async () => {
    const { store } = await (0, mandat_shared_solid_requests_1.getResource)(solidWebPushProfile)
        .then((resp) => resp.data)
        .then((txt) => (0, mandat_shared_solid_requests_1.parseToN3)(txt, solidWebPushProfile));
    const service = store.getSubjects((0, mandat_shared_solid_requests_1.AS)("Service"), null, null)[0];
    const inbox = store.getObjects(service, (0, mandat_shared_solid_requests_1.LDP)("inbox"), null)[0].value;
    const vapidPublicKey = store.getObjects(service, (0, mandat_shared_solid_requests_1.PUSH)("vapidPublicKey"), null)[0].value;
    return { inbox, vapidPublicKey };
};
const _createSubscriptionOnResource = (uri, details) => {
    return `
@prefix rdf: <${(0, mandat_shared_solid_requests_1.RDF)()}> .
@prefix as: <${(0, mandat_shared_solid_requests_1.AS)()}> .
@prefix push: <${(0, mandat_shared_solid_requests_1.PUSH)()}> .
<#sub> a as:Follow;
...
const subscribeForResource = async (uri) => {
    ...
    return (0, mandat_shared_solid_requests_1.createResource)(inbox, solidWebPushSub, session);
};
const unsubscribeFromResource = async (uri) => {
    ...
    return (0, mandat_shared_solid_requests_1.createResource)(inbox, solidWebPushUnSub, session);
};
